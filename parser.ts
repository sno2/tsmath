...
type ParseDelim<T extends string> = T extends `(${infer $Rest}`
  ...
  ? [TokenType.CloseParen, $Rest]
  : never;

type ParseOp<T extends string> =
  //
  T extends `+${infer $Rest}`
    ? [TokenType.OpAdd, $Rest]
    : T extends `-${infer $Rest}`
    ? [TokenType.OpSub, $Rest]
    : T extends `*${infer $Rest}`
    ? [TokenType.OpMul, $Rest]
    : T extends `/${infer $Rest}`
    ? [TokenType.OpDiv, $Rest]
    : never;
...
type Eval<T extends string> = Tokenize<T> extends infer $Tokens
  ? $Tokens
  : never;

type asdf2 = Eval<"255 + 2 / 23">;
